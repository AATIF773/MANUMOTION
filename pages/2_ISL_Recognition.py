...
if not st.session_state.isl_cam_on:
    ...
    with col_pred:
        ...
        count_ph = st.empty()
        stop_btn = st.button("⏹  Stop Webcam", key="isl_stop")
    ...
    def render_letter(ltr, pct):
        if ltr:
            pred_card_ph.markdown(f"""
            <div class="pred-card">
              <div class="pred-lbl">Detected Sign</div>
              <div class='pred-letter'>{ltr}</div>
              <div class="conf-hdr">Confidence</div>
              <div class='conf-track'><div class='conf-fill' style='width:{int(pct)}%;'></div></div>
    ...
    def render_alpha(active):
        cells = "".join(f"<div class='alph-cell{' active' if ch == active else ''}'>{ch}</div>" for ch in ALPHABETS)
        alpha_ph.markdown(f"<div class='alph-grid'>{cells}</div>", unsafe_allow_html=True)

    render_letter("", 0)
    render_alpha("")
    count_ph.markdown("<div class='msb-card'><div class='msb-lbl'>Total Predictions</div><div class='msb-val'>0</div></div>", unsafe_allow_html=True)
                ...
                if result.multi_hand_landmarks:
                    for hlm in result.multi_hand_landmarks:
                        draw_u.draw_landmarks(
                            frame, hlm, mp_h.HAND_CONNECTIONS,
                    ...
                    if conf >= 50.0:
                        ...
                        cv2.rectangle(frame, (0,0),(frame.shape[1],52),(7,7,26),-1)
                        cv2.putText(frame, f"{final_lbl}  {conf:.0f}%",
                                    (12, 38), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (52,211,153), 3)
                    else:
                        cur_letter, cur_conf = "", 0.0
                        cv2.rectangle(frame,(0,0),(frame.shape[1],52),(7,7,26),-1)
                        cv2.putText(frame, f"Searching...  {conf:.0f}%",
                                    (12, 38), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (251,146,60), 2)
                else:
                    cur_letter, cur_conf = "", 0.0
                    cv2.rectangle(frame,(0,0),(frame.shape[1],52),(7,7,26),-1)
                    cv2.putText(frame, "No hand detected",
                                (12, 38), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (80,80,120), 2)

                frame_ph.image(frame, channels="BGR", use_container_width=True)
                render_letter(cur_letter, cur_conf)
                render_alpha(cur_letter)
                count_ph.markdown(f"<div class='msb-card'><div class='msb-lbl'>Total Predictions</div><div class='msb-val'>{total:,}</div></div>", unsafe_allow_html=True)

                if st.session_state.get("isl_stop", False):
                    break
