...
import os
import sys
import av # Added for video processing
from streamlit_webrtc import webrtc_streamer, WebRtcMode, RTCConfiguration

# 1. CLOUD FIX: MOCK PYAUTOGUI
# This prevents the app from crashing on the server since pyautogui needs a screen
...
""", unsafe_allow_html=True)
    ...
    with col_side:
        ...
        st.markdown(f"<div class='sb-card'><div class='sb-lbl'>Hands</div><div class='sb-val'>{n_hands}</div></div>", unsafe_allow_html=True)
        st.markdown("<div class='sb-card'><div class='sb-lbl'>Status</div><div class='sb-val'>Active</div></div>", unsafe_allow_html=True)
        st.markdown(f"<div class='sb-card'><div class='sb-lbl'>Active Gesture</div><div class='sb-gesture'>{gesture_text if gesture_text else '—'}</div></div>", unsafe_allow_html=True)
